...
export const MoreDropdown = ({ options, selectedDoc, onSelect, isOpen, setIsOpen }) => {
  ...
  const [selectedOption, setSelectedOption] = useState(selectedDoc); // Initialize with selectedDoc
  ...
  return (
    <>
      ...
      >
        <span>{selectedOption ? selectedOption.name : "More"}</span> {/* Display selected option */}
        {isOpen ? (
          ...
          >
            <div className="py-1">
              {options?.map((option, index) => (
                <button
                  key={index}
                  onClick={() => handleSelect(option)} // Use handleSelect function
